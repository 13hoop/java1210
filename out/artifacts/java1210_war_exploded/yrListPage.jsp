...
<div class="container">
    ...
    <form id="selectedForm" action="${pageContext.request.contextPath}/deleteSeriesServlet" method="post">
        <table border="1" class="table table-bordered table-hover">
            ...
            <c:forEach items="${pData.list}" var="user" varStatus="s">
                <tr>
    ...
    <nav aria-label="Page navigation example">
        <ul class="pagination justify-content-center">
            ...
            <li class="page-item">
                <a class="page-link" href="${pageContext.request.contextPath}/queryUserPage?page=${pData.currPageNum-1}" tabindex="-1" aria-disabled="true">Previous</a>
            </li>

            <c:forEach begin="1" end="${pData.totalPageNum}" var="pNum" varStatus="s">
                <c:if test="${pData.currPageNum == pNum}">
                    <li class="page-item active" ><a class="page-link" href="${pageContext.request.contextPath}/queryUserPage?page=${pNum}">${pNum}</a></li>
                </c:if>
                <c:if test="${pData.currPageNum != pNum}">
                    <li class="page-item"><a class="page-link" href="${pageContext.request.contextPath}/queryUserPage?page=${pNum}">${pNum}</a></li>
                </c:if>
            </c:forEach>

            <li class="page-item">
                <a class="page-link" href="${pageContext.request.contextPath}/queryUserPage?page=${pData.currPageNum + 1}">Next</a>
            </li>

            <span style="font-size: 25px;margin-left: 5px;">
                    共${pData.wholeCount}条记录，共${pData.totalPageNum}页
            </span>
        </ul>
    </nav>
</div>
